...
The static plate is `rgb_mask_sweep.py`'s qualitative half: ONE validation
...
Provenance notes that matter, and their limits:
  ...
  * checkpoint. rgb_mask_sweep.py hard-codes ./outputs/4m_distill_15k_all/
...
CKPT = 'outputs/4m_distill_15k_all/best_model.pth'      # rgb_mask_sweep.py:20
LEVELS = [0.0, 0.20, 0.50, 0.80, 0.95]                  # rgb_mask_sweep.py:22
PLANT = 'Sorghum_10001_00'                              # rgb_mask_sweep.py:23
...
def main():
    ...
    out = {
        ...
        'rows': rows,
        'note': (f'rgb_mask_sweep.py on {nm}, val split, all-source distilled generalist '
