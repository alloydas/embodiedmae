"""MAIZE 4M pretraining — the maize twin of train_sorghum_4m.py.

Deliberately a separate entry point: maize and sorghum are kept as
independent pipelines so either can change without touching the other.
The diff against the sorghum script is small and confined to four things —
the model factories (embodied_mae_4m_maize_*), the dataset (MaizeDataset4M),
max_leaves (28 vs 24) and num_points (8192 vs 8196, because a maize
pointcloud_cam.ply holds exactly 8192 points and asking for more silently
pads with duplicates). Everything else — the loop, DDP, checkpointing,
the visualisation grid — is shared logic that happens to be species-neutral.
"""
import os
import json
import argparse
...
def config_to_namespace(config):
    ...
    ns.num_points         = config['data'].get('num_points', 8192)
    ...
    # see MaizeDataset4M). None -> every plant. val/test are never capped.
    ...
    ns.max_leaves         = config['model'].get('max_leaves', MAX_LEAVES)
...
def main():
    ...
    parser.add_argument('--config',             type=str,   default='configs/config_maize.yaml')
