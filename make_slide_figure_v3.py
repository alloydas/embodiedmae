"""Build a multi-plant slide figure for EmbodiedMAE-4M run v3.

Reads from a `validate_4m.py` dump (`slide_dump_v3/sample_<name>/`) and writes
`slide_figure_v3.png`. Shows ≥15 plants, all four modalities (RGB / Depth /
PointCloud / Procedural params), and an aggregate-metrics caption.

Layout (landscape 16:9-ish):

    Row 0: title + best-checkpoint sub-caption + aggregate metrics strip
    Row 1: RGB ground-truth     × N plants
    Row 2: RGB input (masked)   × N plants
    Row 3: RGB reconstruction   × N plants
    Row 4: Depth GT             × N plants
    Row 5: Depth reconstruction × N plants
    Row 6: PC reconstruction    × N plants  (3D scatter, top/side combined)
    Row 7: per-plant metric strip (rgb_mse / pc_chamfer / param_mae)
"""
...
def main():
    ...
    ap.add_argument('--dump_dir',   default='slide_dump_v3')
    ap.add_argument('--out',        default='slide_figure_v3.png')
    ap.add_argument('--n_plants',   type=int, default=15)
    ap.add_argument('--dpi',        type=int, default=140)
    ap.add_argument('--best_epoch', type=int, default=1100)
    ap.add_argument('--total_epochs', type=int, default=2400)
    ap.add_argument('--cur_epoch',  type=int, default=1125,
                    help='Last completed epoch in the run')
    args = ap.parse_args()
    ...
    # ── Figure layout ────────────────────────────────────────────────────────
    # 7 modality rows × n plants, plus a title row and a metrics-strip row.
    row_labels = [
        'RGB ground-truth',
        'RGB input (masked)',
        'RGB reconstruction',
    ...
    # Compose figure size based on n_plants
    col_w = 1.75               # inches per plant column
    fig_w = max(22, 2.5 + n * col_w)
    fig_h = 16.5
    fig = plt.figure(figsize=(fig_w, fig_h))

    # GridSpec: 1 title row + n_rows of image rows + 1 metrics strip
    ...
    title_ax.text(
        0.5, 0.88,
        f'EmbodiedMAE-4M  ·  {n}-plant reconstruction gallery  ·  run v3',
        ha='center', va='center', fontsize=26, weight='bold',
        transform=title_ax.transAxes,
    )
    sub = (f'best checkpoint epoch {args.best_epoch} (val loss 0.1268)   ·   '
    ...
    # Progress bar (placed below the sub-caption so they don't overlap)
    frac = min(max(args.cur_epoch / args.total_epochs, 0.0), 1.0)
    ...
    title_ax.text(
        0.5, bar_y + bar_h / 2,
        f'training stopped early at epoch {args.cur_epoch} / {args.total_epochs}  '
        f'({frac*100:.0f}%) — SLURM wall-time',
    ...
    for c, s in enumerate(samples):
        # Row 0 (top header on each col): plant name
    ...
    # Aggregate metrics from the dump (this evaluation, 15 plants @ mask=0.4)
    agg_text = (
    ...
    # Best-epoch metrics from the actual training history
    best_text = (
        f'Best epoch (training val, mask=0.15) ep {args.best_epoch}       '
        'val_loss = 0.1268     '
        'RGB MSE = 0.6804     '
        'Depth MSE = 0.0085     '
        'PC Chamfer = 0.0011     '
        'Param MAE (masked) = 0.0106     '
        'Param acc@0.05 = 0.9713'
    )
