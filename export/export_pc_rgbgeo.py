...
figures/make_rgb2pc_figure.py) with the raw 3D points behind it, so the results page can
spin each cloud instead of showing one fixed matplotlib camera.

Provenance is copied from figures/make_rgb2pc_figure.py verbatim:
...
One thing cannot be copied: figures/make_rgb2pc_figure.py never seeds numpy, and
...
CKPT = 'outputs/4m_distill_15k_all/best_model.pth'   # as in figures/make_rgb2pc_figure.py
...
def main():
    ...
    out = dict(
        ...
        note=('Same four plants and the same hard-coded checkpoint as '
              'figures/make_rgb2pc_figure.py (outputs/4m_distill_15k_all/best_model.pth, '
