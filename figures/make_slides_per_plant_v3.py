...
# ── helpers (cloned/adapted from figures/make_slide_figure.py) ───────────────────────
...
def render_plant(d, out_path: Path, *,
                 best_epoch=1100, max_leaves=6, dpi=160):
    ...
    tax.axis('off')
    tax.text(0.5, 0.78,
             f'EmbodiedMAE-4M  ·  plant {name}  ·  best epoch {best_epoch} (val 0.1268)',
...
CURVES_PATH = Path('/work/mech-ai-scratch/alloy/embodiedmae/outputs/4m_run_v3/loss_curves.png')
...
def add_arch_slide(prs, blank, n_plants):
    ...
    if ARCH_PATH.exists():
        ...
        add_textbox(s, Inches(0.6), Inches(4.45), Inches(12.4), Inches(0.6),
                    '(arch_diagram.png missing — regenerate with figures/make_arch_diagram.py)',
...
def build_pptx(png_paths, samples, out_path, agg=None):
    ...
    for png, samp in zip(png_paths, samples):
        ...
        m = samp['metrics']
        notes.text = (
            f'Plant {samp["name"]}  (run v3, best epoch 1100)\n\n'
            f'Mask fractions:\n'
...
def main():
    ...
    ap.add_argument('--out_dir',   default='slide_per_plant_v3')
    ap.add_argument('--pptx',      default='slide_figure_per_plant_v3.pptx')
