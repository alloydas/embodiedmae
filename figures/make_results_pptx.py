...
this runs in `det` (which has python-pptx) rather than `det_cu128`:
    ...
    vis_unpredicted/clouds.json   <- eval/vis_pc_unpredicted.py --export_json
    <gallery>/views.json          <- eval/eval_views_one_plant.py --export_gallery
...
change them in one place and they disagree, so change both.
    ...
    conda activate det
    python figures/make_results_pptx.py --gallery <dir-with-views.json>
"""
